...
This notebook accompanies Scenario 2 in Kumle, Vo & Draschkow (in preparation).  
...
First, we will provide some general information about the model we have chosen to simulate power for. Note how we specified "sentence" as the random effect we want to vary.
```{r eval=FALSE}
...
Similar to Scenario 1, we will also include the SESOI (i.e. smallest effect of interest) specification. An introduction to the SESOI can be found in Notebook 1 and the tutorial paper (Kumle, Vo & Draschkow, in preparation).
```{r eval=FALSE}
