---
title: 'Scenario 2: Simulating different units and possible combinations'
output:
  html_document:
    df_print: paged
...
This Notebook aims at accompanying Scenario 2 in Kumle, Vo & Draschkow (in preparation).  

For a general introduction to simulation-based power analyses as well as using simulations to explore power for different sample sizes, see [Notebook 1](https://lkumle.github.io/power_notebooks/Scenario1_notebook.html). 

In this Noteboook, we will cover situations in which we wish to simulate power for random effects other than subject. To do so, the packages [*mixedpower* ](https://github.com/DejanDraschkow/mixedpower) (Kumle, Vo & Draschkow, 2018) and [*simr*](https://besjournals.onlinelibrary.wiley.com/doi/full/10.1111/2041-210X.12504) (Green & Macleod, 2016) are used and all simulations are based on (generalized) mixed models fitted with [*lme4*](https://arxiv.org/pdf/1406.5823.pdf) (Bates, Maechler, Bolker & Walker, 2015). All simulations shown in this Notebook are applicable for linear mixed models as well as generalized linear mixed models. 

See [Notebook 3](https://lkumle.github.io/power_notebooks/Scenario3_notebook.html) for starting a simulation-based power estimation from scratch.   
&nbsp;
...
As in [Notebook 1](https://lkumle.github.io/power_notebooks/Scenario1_notebook.html), we will base our simulation on data published by Yan, Zhou, Shu, Yusupu, Miao, Kruegel & Kliegl (2014) retrieved from http://read.psych.uni-potsdam.de. A preprocessed data set can be downloaded [here](https://www.dropbox.com/s/ply8xkg123d4ee0/Yan_et_al.RData?dl=0). 
...
Yan et all. (2014) tested 48 subjects, each of whom read 120 sentences investigating the effect of different factors on the first landing position while reading (FLP, i.e. the position in a sentence your eyes first land on) - a more detailed introduction to the data set can be found in the general tutorial and Notebook 1 where we varied the number of subjects to explore the effect on power. The levels of the second random parameter (i.e. stimuli/items), however, have been kept constant even though it is of course possible to also change the levels of the second random effect.

&nbsp;  

Again, we are interested in the effect of morphological complexity (i.e. sumber of suffixes) and word length and therefore will include them as fixed effects in our model fitted with *lme4*:
...
#### **mixedpower**
Using the mixedpower package, all that has to be changed conpared to Scenario 1 is the specification of **simvar** and its **steps**. However, in the following we will again specify all parameters before running the actual power analysis.
...
First, we will provide some general information about the LMM we want to simulate power for to set the foundation for the power simulation about to be performed. Note how we specified "sentence" as the random effect we want to vary. 
```{r eval=FALSE}
...
simvar <- "sentence" # which random effect do we want to vary in the simulation? 
...
Similar to Scenario 1, we will include the SESOI (i.e. smallest effect of interest) simulation.  An introduction to the SESOI can be found in Notebook 1 and the tutorial paper (Kumle, Vo & Draschkow, in preparation). 
```{r eval=FALSE}
...
Finally, we will combine all those information in the mixedpower()-function and run the power analysis. Depending on the complexity and size of the model and data as well as on the number of cores on the machine used for simulation … time to take a short break! Running on a machine with 6 cores, the following simulation will take approximatly 10-15 minutes. 
```{r eval=FALSE}
...
load("~/Dropbox/Power/manuscript/BRM/analyses_BRM/Scenario 2/R2_scenario2_48subjects_new.Rdata")
...
***
***
### Varying the levels multiple random effects simultaneously

So far, we only varied the levels of one random effect while we kept the other one constant. However, being able to vary both simultaneously would be desirable as it allows to estimate power for different combinations of random effect levels.
...
However, the simulation of new data sets now follows a two-step simulation process where the levels of one random effect are simulated before the levels of the second random effect are adapted. To do so, we need to specify the desired levels and names of  *both* random effects. 

Imagine wanting to explore power for a range of sentences - but not for 48 subjects like in the data published by Yan et al. (2014) but for 30 subjects. Consequentially, we would keep sentences as our “simvar” and add subject as the second varying random effect. All other parameters will stay the same as in the simulation run with mixedpower() above. 
